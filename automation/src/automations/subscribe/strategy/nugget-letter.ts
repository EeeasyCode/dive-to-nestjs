...
export class NuggetLetter implements SubscribeStrategy {
  async subscribe(email: string, nickname: string) {
    ...
    try {
      ...
      await page.click(
        '#stb_subscribe > form > div:nth-child(8) > label > input[type=checkbox]',
      );
      await page.click(
        '#stb_subscribe > form > div:nth-child(9) > label > input[type=checkbox]',
      );
      await page.click(
        '#stb_subscribe > form > div.stb_form_set_submit > button',
      );
